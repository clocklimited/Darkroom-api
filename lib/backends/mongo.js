var mongo = require('mongodb')
  ...
  , fs = require('fs')
  , allowType = require('../type-whitelist')
  , initCreateWriteStream = require('./writestream-creator')

module.exports = function (config, cb) {
  console.log('Connecting to', config.databaseUri)
  mongo.MongoClient.connect(config.databaseUri, function (err, db) {
    if (err) return cb(err)

    var gfs = new Grid(db, mongo)
      , factory = { _db: db, _gfs: gfs }
    ...
    factory.createDataWriteStream = initCreateWriteStream(temp, createTypeDetector)
    ...
    factory.isHealthy = function isHealthy(cb) {
      db.collection('users').find({}).toArray(function (err) {
        if (err) {
          return cb(err, false)
        }
        cb(null, true)
